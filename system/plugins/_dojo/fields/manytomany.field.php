...
class zajfield_manytomany extends zajField {
	...
	public function save($data, &$object, $additional_fields=false){
		...
		// is data is a comma separated string
		 	elseif(is_string($data)){
			 	$othermodel = $this->options['model'];
				// build new array
			 		$new_entries = explode(',', $data);
		 		// build existing array with current value
		 			$existing_entries = array();
			 		//foreach($object->data->$field_name as $id=>$object) $existing_entries[] = $id;
			 	// check which values are in new but not in existing: add
			 		$add_values = array_diff($new_entries, $existing_entries);
			 	// check which values are in existing but not in new: remove
			 		$remove_values = array_diff($existing_entries, $new_entries);
			 	// perform add
			 		if(count($add_values)){
			 			foreach($add_values as $id){
		 					$otherobject = $othermodel::fetch($id);
		 					// TODO: if false, it doesnt exist...create?
					 		if($otherobject) $object->data->$field_name->add($otherobject, 'add', $additional_fields);
			 			}
			 		}
				// perform remove
			 		if(count($add_values)){
			 			foreach($add_values as $id){
		 					$otherobject = $othermodel::fetch($id);
					 		if($otherobject) $object->data->$field_name->remove($otherobject, 'remove', $additional_fields);
			 			}
			 		}				
		 	}
